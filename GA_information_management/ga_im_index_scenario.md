...
## Step 1: Create an draft event schema file
{: #step1}
...
For more details, see the [{{site.data.keyword.iot_short_notm}} HTTP REST API](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002/info-mgmt.html#!/Schemas) documentation.
...
```
{
  ...
  "refs" : {
      "content" : "draft/schemas/5846cd7c6522050001db0e0d/content"
  },
...
```
{
  ...
  "refs" : {
      "content" : "draft/schemas/5846cee36522050001db0e0e/content"
  },
  "contentType" : "application/octet-stream",
  "createdBy" : "a-8x7nmj-9iqt56kfil"
}
...
For more details, see the [{{site.data.keyword.iot_short_notm}} HTTP REST API](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002/info-mgmt.html#!/Event_Types) documentation.
...
```
{
  ...
  "refs" : {
    "schema" : "draft/schemas/5846cd7c6522050001db0e0d"
  },
  "name" : "tEvent",
...
```
{
  ...
  "name" : “tempEvent”,
  "refs" : {
    "schema" : "draft/schemas/5846cee36522050001db0e0e"
  },
  "updatedBy" : "a-8x7nmj-9iqt56kfil"
}
```
...
For more details, see the [{{site.data.keyword.iot_short_notm}} HTTP REST API](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002/info-mgmt.html#!/Physical_Interfaces) documentation.
...
```
{
  "updatedBy" : "a-8x7nmj-9iqt56kfil",
  "refs" : {
    "events" : "draft/physicalinterfaces/5847d1df6522050001db0e1a/events"
  },
...
```
{
  "updatedBy" : "a-8x7nmj-9iqt56kfil",
  "refs" : {
    "events" : "draft/physicalinterfaces/5847d1df6522050001db0e1b/events"
  },
...
For more details, see the [{{site.data.keyword.iot_short_notm}} HTTP REST API](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002/info-mgmt.html#!/Device_Types) documentation.
...
```
{
  ...
  "refs" : {
    ...
    "logicalInterfaces" : "/device/types/EnvSensor1/logicalinterfaces",
...
```
{
  ...
  "refs" : {
    ...
    "logicalInterfaces" : "/device/types/EnvSensor2/logicalinterfaces",
...
schemaId	|	The id created for the logical interface schema resource.


For more details, see the [{{site.data.keyword.iot_short_notm}} HTTP REST API](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002/info-mgmt.html#!/Logical_Interfaces) documentation.
...
```
POST /device/types/{typeId}/logicalinterfaces
```
...
Parameter	|	Description
------	|	-----
id	|	The id created for the logical interface.
name	|	The name of the device type.
schemaId	|	The id created for the logical interface resource.
...
```
curl --request POST \
--url https://yourOrgID.internetofthings.ibmcloud.com/api/v0002/device/types/EnvSensor1/logicalinterfaces \
--header 'authorization: Basic MK2fdJpobP6tOWlhgTR2a4Hklss2eXC7AZIxZWxPL9B8XlVwSZL=' \
--header 'content-type: application/json' \
...
```
curl --request POST \
--url https://yourOrgID.internetofthings.ibmcloud.com/api/v0002/device/types/EnvSensor2/logicalinterfaces \
--header 'authorization: Basic MK2fdJpobP6tOWlhgTR2a4Hklss2eXC7AZIxZWxPL9B8XlVwSZL=' \
--header 'content-type: application/json' \
--data '{"createdBy" : "a-8x7nmj-9iqt56kfil", \
          "refs" : {
...
Parameter	|	Description
------	|	-----
logicalInterfaceId	|	The id created for the logical interface.
propertyMappings	|	A valid JSON structure that maps properties defined for the logical interface with the properties of the device event payload. See the following examples.
...
curl --request POST \
  ...
  --header 'authorization: Basic MK2fdJpobP6tOWlhgTR2a4Hklss2eXC7AZIxZWxPL9B8XlVwSZL=' \
  --header 'content-type: application/json' \
  --data '{"logicalInterfaceId" : "5846ed076522050001db0e12","propertyMappings" : {
              "tevt" : {
                  "temperature" : "$event.t"
              }
            }
...
To validate your draft device type configuration, use the following API:
...
In this scenario, we need to activate configuration for two draft device types.

The following example shows how to use cURL to validate your configuration for draft device type *EnvSensor1*:
...
```
{
 "message": "CUDRS0520I: State update configuration for device type 'EnvSensor1' has been successfully submitted for validation",
  "details": {
    "id": "CUDRS0520I",
...
```
The following example shows how to use cURL to activate your validated configuration for draft device type *EnvSensor1*:
...
```
The following example shows how to use cURL to validate your configuration for draft device type *EnvSensor2*:
...
```
{
 "message": "CUDRS0520I: State update configuration for device type 'EnvSensor2' has been successfully submitted for validation",
  "details": {
    "id": "CUDRS0520I",
...
```
The following example shows how to use cURL to activate your configuration for device type *EnvSensor2*:
...
**Note:** To retrieve the latest active configuration, use the following API:
