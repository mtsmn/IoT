...
## 인증서 폐기 목록 및 클라이언트 인증서
{: #CRLs}

인증서 폐기 목록(CRL)은 클라이언트 인증서를 사용하여 디바이스를 인증하는 구성을 위해 지원됩니다. CRL 분배 지점으로 지정된 위치는 HTTP 또는 HTTPS 액세스를 지원해야 하며, 해당 사이트는 접속 가능해야 합니다. CRL의 분배 지점에 접속할 수 없는 경우 또는 CRL이 유효하지 않거나 분배 지점에서 이를 찾을 수 없는 경우에는 클라이언트 연결이 거부됩니다.  
 

클라이언트 인증서에는 "X509v3 확장기능"의 "X509v3 키 사용" 섹션에 지정된 "인증서 서명"이 있는 인증 기관(CA) 인증서의 서명이 있어야 합니다. CRL을 사용하여 클라이언트 인증서를 취소하려면, 클라이언트 인증서를 서명하는 CA 인증서에 "X509v3 확장기능"의 "X509v3 키 사용" 섹션의 "CRL 서명" 역시 포함되어야 합니다. 이 구성을 사용하여 CA 인증서는 필요 시에 CRL 업데이트를 발행하고 서명할 수 있습니다. 오직 클라이언트 인증서 분배 지점에 지정된 CRL만 인증서가 취소되었는지 여부를 판별하는 데 사용됩니다.

반드시 인증서 및 CRL을 테스트하십시오(특히, 공인된 인증 기관의 인증서를 사용하는 대신 자체 인증서 및 CRL을 생성하는 경우). 
...
**ASP(Advanced Security Plan) 또는 Lite 플랜:**
- 시스템 운영자는 다음 옵션을 사용하여 연결 정책을 구성할 수 있습니다.
    - TLS 선택적
    - TLS와 토큰 인증
    - TLS와 클라이언트 인증서 인증
