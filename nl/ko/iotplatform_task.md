...
1. IBM Cloud 콘솔의 메뉴에서 **IoT**를 선택하고 {{site.data.keyword.iot_short_notm}} 링크를 클릭하십시오.   
2. 로그인하거나 **회원가입하기**를 클릭하십시오.   
3. {{site.data.keyword.iot_short_notm}} 페이지에서 지역, 조직 및 영역을 선택하고 **작성**을 클릭하십시오.   
...
3. 개요 대시보드의 메뉴 분할창에서 **디바이스**를 선택한 다음 **디바이스 추가**를 클릭하십시오.
5. 추가할 디바이스의 디바이스 유형을 선택하거나 작성하십시오.  
...
10. **다음**을 클릭하여 선택한 디바이스 유형의 디바이스를 추가하는 프로세스를 시작하십시오.
11. 디바이스 ID(예: `my_first_device`)를 입력하십시오.  
...
12. 선택사항: **추가 필드**를 클릭하여 일련 번호, 제조업체, 모델 등의 디바이스 정보를 추가하십시오.  
 **팁:** 이 정보는 나중에 추가하고 편집할 수 있습니다.
12. 선택사항: 디바이스 JSON 메타데이터를 입력하십시오.  
 **팁:** 디바이스 메타데이터는 나중에 추가하고 편집할 수 있습니다.
13. **다음**을 클릭하여 디바이스의 추가를 완료하십시오.
14. 요약 정보가 올바른지 확인한 다음 **추가**를 클릭하여 연결을 추가하십시오.  
...
15. 디바이스 정보 페이지에서 다음 디바이스 정보를 복사하고 저장하십시오.  
 - 조직 ID(예: `tubo8x`)
...
**팁:** 일반적으로 사용되는 디바이스에 여러 연결 레시피를 사용할 수 있습니다. 레시피의 목록은 IBM.com에서 사용 가능한
...
**중요:** {{site.data.keyword.iot_short_notm}}의 디바이스 복원 기능은 제한된 베타 프로그램의 일부로서만 사용 가능합니다. 향후 업데이트에는 이 기능의 현재 버전과 호환 가능한 변경사항이 포함될 수 있습니다. 시도해 보고 [의견을 보내주십시오. ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://developer.ibm.com/answers/smart-spaces/17/internet-of-things.html){: new_window}

디바이스를 실수로 삭제한 경우 14일 이내에 복원할 수 있습니다. 

디바이스가 삭제되면 디바이스 "멘토"가 작성됩니다. 멘토는 디바이스 문서의 사본이며 삭제된 후 14일 동안 사용 가능합니다.

다음 디바이스 복원 API는 디바이스의 이전 버전을 복원하기 위해 멘토를 사용할 수 있게 합니다.

    POST /archive/device/types/{typeId}/devices/{deviceId}/restore


다음 API를 사용하여 전체 디바이스 멘토 목록을 검색할 수 있습니다.

    GET /archive/device/types/{typeId}/devices/{deviceId}

디바이스 복원 API에 대한 자세한 정보는 [디바이스 복원 API 베타 ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")](https://docs.internetofthings.ibmcloud.com/apis/swagger/v0002-beta/restore-device-beta.html)를 참조하십시오.

## 디바이스 다시 연결

네트워크 문제점 또는 서비스나 인프라의 일상적인 유지보수로 인해 {{site.data.keyword.iot_short_notm}}에서 디바이스 연결이 끊어졌습니다. 디바이스가 다시 연결되면 재연결 동안 생성된 네트워크 트래픽 양을 최소화할 것을 고려하거나 동시 재연결 수를 줄이기 위해 시간-지연을 도입하려고 할 수 있습니다. 
