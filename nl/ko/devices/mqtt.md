...
관리 디바이스는 서비스 품질(QoS) 레벨이 0, 1 또는 2인 메시지를 공개할 수 있습니다. 

QoS=0인 메시지는 버릴 수 있으며 메시징 서버가 다시 시작된 후 유지되지 않습니다. QoS=1인 메시지는 큐에 넣을 수 있으며 메시징 서버가 다시 시작된 후 유지됩니다. 구독 지속성에 따라 요청을 큐에 넣을지가 판별됩니다. 구독한 연결의 ``cleansession`` 매개변수에 따라 구독 지속성이 판별됩니다. 디바이스가 QoS2에서 메시지를 공개할 수 있지만 디바이스 관리 서버가 QoS1에서 공개하고 구독하므로, 응답 메시지는 오직 QoS1 뿐입니다. 
