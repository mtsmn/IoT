...
{{site.data.keyword.iot_full}} 将设备和网关识别为两类设备。设备类使用“classId”字段来标识。设备类中的设备可以是受管设备或非受管设备。
...
设备管理协议定义了一组受支持的操作。设备管理代理程序可以支持其中一部分操作，但它必须发出“管理设备”请求。支持固件操作的设备还必须支持观察。
...
2. 设备连接到 {{site.data.keyword.iot_short_notm}} 并发出“管理设备”请求以成为受管设备。
3. 可以使用 {{site.data.keyword.iot_short_notm}} REST API 来查看和处理设备的元数据。这些 API 操作（例如，固件更新和设备重新启动）在[设备管理请求](https://console.ng.bluemix.net/docs/services/IoT/devices/device_mgmt/requests.html)文档中进行了概述。
4. 设备可以使用设备管理协议传达有关其位置、诊断信息和错误代码的更新。
5. 设备退役后，可以使用仪表板或 REST API 将其从 {{site.data.keyword.iot_short_notm}} 中除去。
...
设备使用“管理设备”请求来变成受管设备。设备管理代理程序必须先发送“管理设备”请求，然后才能接收来自服务器的请求。每当设备管理代理程序启动或重新启动后，通常都会发送此类型的请求。
...
|403|禁止（如果设备尝试发布声明支持一组无效操作的管理请求）。|
|404|设备尚未向 {{site.data.keyword.iot_short_notm}} 注册。|
|409|资源由于冲突而无法更新（例如，两个请求同时在更新该资源）。可以稍后重试更新。|
...
设备不再需要进行管理时，可以使用“取消管理设备”请求。设备变为非受管时，{{site.data.keyword.iot_short_notm}} 不会再向该设备发送新的设备管理请求。非受管设备可以继续发布错误代码、日志消息和位置消息。
...
设备的位置元数据可以通过以下方式在 {{site.data.keyword.iot_short_notm}} 中进行更新：

### 自动设备位置更新
- 可以确定其位置的设备可以选择向 {{site.data.keyword.iot_short_notm}} 设备管理服务器通知有关位置更改的信息。设备将向 {{site.data.keyword.iot_short_notm}} 通知有关位置更新的信息。设备检索其位置（例如，从 GPS 接收器进行检索），然后向 {{site.data.keyword.iot_short_notm}} 实例发送设备管理消息以更新其位置。时间戳记会捕获从 GPS 接收器检索到位置的时间。即便发送位置更新消息有所延迟，该时间戳记也仍有效。服务器会记录消息接收的日期和时间，并使用该信息更新位置元数据（如果未使用时间戳记）。
...
用户或应用程序更新活动受管设备的位置时，该设备会收到更新消息。
...
``measuredDateTime`` 字段是测量位置的日期和时间。

每当更新位置时，为纬度、经度、海拔和准确性提供的值会视为单次多值更新。纬度和经度是必需项，每次更新时都必须提供这两项。必须使用 1984 年世界大地坐标系 (WGS84) 以十进制度数指定纬度和经度。海拔和准确性是可选的，以米为测量单位。
...
|200|操作成功。|
|400|输入消息与预期格式不匹配，或者其中一个值不在有效范围内。|
|404|设备尚未向 {{site.data.keyword.iot_short_notm}} 注册。|409|资源由于冲突而无法更新（例如，两个请求同时在更新该资源）。可以稍后重试更新。|
...
如果 `rc` 参数的值不是 `200`，那么可以指定响应中使用的 `message` 参数。如果无法检索到任何指定的参数值，那么必须将 `rc` 参数的值设置为 `404`（如果找不到属性）或 `500`（对于其他任何原因）。找不到参数的值时，`fields` 数组应该包含其 `field` 设置为无法读取的每个参数的名称的元素。应该忽略 `value` 参数。要使响应代码参数设置为 `200`，必须指定 `field` 和 `value`，其中 `value` 是 `field` 参数值标识的属性的当前值。
...
如果 `rc` 参数的值不是 `200`，那么可以指定 `message` 参数。
...
`field` 参数的值是已更改属性的名称，`value` 是该属性的当前值。属性可以为复杂字段。如果复杂字段中的多个值作为单个操作的结果进行了更新，那么仅发送单个通知消息。
