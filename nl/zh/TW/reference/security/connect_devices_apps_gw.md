...
確定所需的埠已開啟並已啟用，可進行通訊。埠 8883 及 443 支援搭配使用傳輸層安全 (TLS) 與 MQTT 及 HTTP 通訊協定的安全連線。埠 1883 支援使用 MQTT 及 HTTP 通訊協定的不安全連線。依預設，會停用 HTTP 埠 1883。如需變更預設值的相關資訊，請參閱[配置安全原則](set_up_policies.html#set_up_policies.html)。
...
如需共用訂閱格式的相關資訊，請參閱[應用程式的 MQTT 連線功能](../../applications/mqtt.html)。
