...
ゲートウェイ・デバイスはデバイスの特殊なクラスであり、他のデバイスの代理として動作する権限を持ちます。 ゲートウェイ・リソース・グループは、各ゲートウェイが組織内のどのデバイスの代理として動作するのかを定義します。 ゲートウェイ役割により、ゲートウェイがデバイスを Watson IoT Platform に登録できるかどうかが決まります。
{: #shortdesc}

リソース・グループを使用して、ゲートウェイが組織のデバイスのサブセットの代理として機能できるようにします。ゲートウェイは、そのゲートウェイ自体とリソース・グループ内のデバイスの代理としてのみ、メッセージをパブリッシュまたはサブスクライブすることができます。 

新しいゲートウェイが作成されると、デフォルトのリソース・グループも作成され、ゲートウェイに割り当てられます。ゲートウェイからデフォルトのリソース・グループを削除することはできません。リソース・グループを持たない既存のゲートウェイは、引き続き組織内のすべてのデバイスの代理として機能できます。

API を使用してゲートウェイ・デバイスからイベントをパブリッシュする方法については、[ゲートウェイ・デバイス用の HTTP API](gw_api.html) を参照してください。

## ゲートウェイへの役割の割り当て
{: #gw_roles}

ゲートウェイをリソース・グループに割り当てるには、最初にゲートウェイを役割に割り当てる必要があります。新しく作成されたゲートウェイには、デフォルトで*特権ゲートウェイ* 役割が割り当てられ、デフォルトのリソース・グループに割り当てられます。特権役割を持つゲートウェイは、デバイスを自動登録でき、これらのデバイスはデフォルトのリソース・グループに追加されます。

*標準ゲートウェイ* 役割のゲートウェイは、デバイスを自動登録できません。 

ゲートウェイにリソース・グループが割り当てられると、その役割が変更されても、ゲートウェイ自体とそのリソース・グループ内のデバイスの代理としてのみ動作できます。

ゲートウェイ役割について詳しくは、[ゲートウェイ役割](../roles_index.html#gateway_roles)を参照してください。
