...
3. ゲートウェイ診断情報にアクセスします。  
 ...
 `MsgInRate` | 直近の 1 分間に EAA に送信された 1 秒あたりの推定メッセージ数。  
 ...
 `IsAlive` | `LastHeartBeat` と `CurrentTimestamp` の差が 20 秒より大きい場合、このプロパティーは 0 です。
 `BytesOutCount` | EAA から {{site.data.keyword.iot_short}} に送信されたメッセージのバイト数。
 `BytesOutRate` | 直近の 1 分間に EAA から {{site.data.keyword.iot_short}} に送信された 1 秒あたりのメッセージの推定バイト数。
 `BytesInCount` | {{site.data.keyword.iot_short}} から EAA に送信されたメッセージのバイト数。
 `BytesInRate` | 直近の 1 分間に {{site.data.keyword.iot_short}} から EAA に送信された 1 秒あたりのメッセージの推定バイト数。
 `RuleBytesInCount` |EAA ルール・エンジン・コアに送信されたメッセージのバイト数。 </br> **注:** デバイス・タイプにルールが設定されていない場合、そのデバイス・タイプのメッセージは、ルール・エンジン・コアに送信されません。
 `RuleBytesInRate` | 直近の 1 分間に EAA ルール・エンジン・コアに送信された 1 秒あたりのメッセージの推定バイト数。
 `MsgOutCount` | EAA から {{site.data.keyword.iot_short}} に送信されたメッセージ数。
 `MsgOutRate` | 直近の 1 分間に EAA から {{site.data.keyword.iot_short}} に送信された 1 秒あたりのメッセージの推定バイト数。
 `MsgReducePercent` | 着信メッセージ数と発信メッセージ数の差の比率。 </br>計算には、以下の数式が使用されます。`(msgIn - msgOut) / msgIn`
 `BytesReducePercent` | 着信バイト数と発信バイト数の差の比率。 </br>計算には、以下の数式が使用されます。`(bytesIn - bytesOut) / bytesIn`
 `MsgRateReduce` | 着信メッセージ・レートと発信メッセージ・レートの差の比率。 </br>計算には、以下の数式が使用されます。`(msgInRate - msgOutRate) / msgInRate`
 `BytesRateReduce` | 着信メッセージ・バイト・レートと発信メッセージ・バイト・レートの差の比率。 </br>計算には、以下の数式が使用されます。`(bytesInRate - bytesOutRate) / bytesInRate`
 `SystemLoad` | EAA が実行されているシステムの現在のシステム負荷。 **注:** CPU レートは、EAA が実行されているシステムで `mpstat` コマンドが使用可能な場合にのみ送信されます。 それ以外の場合は、最近 1 分間のシステム負荷の平均が送信されます。 </br>「システム負荷の平均は、使用可能なプロセッサーのキューに入れられている実行可能なエンティティーの数と、使用可能なプロセッサーで実行されている実行可能なエンティティーの数の合計の一定期間の平均値です。 負荷平均を計算する方法はオペレーティング・システムに固有ですが、通常は、ダンプ時間に依存する平均です。 負荷平均が使用できない場合には、負の値が戻されます。 」- _ManagementFactory.getOperatingSystemMXBean_ の javadoc。
