...
次の図は、{{site.data.keyword.iot_full}} エッジ分析環境の一般的なアーキテクチャーを示しています。![エッジ分析アーキテクチャーの IBM Watson IoT プラットフォーム](images/architecture_platform_edge.svg "エッジ分析アーキテクチャーを採用した IBM Watson IoT プラットフォーム")
