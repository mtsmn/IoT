...
Os planos de serviços pagos do {{site.data.keyword.iot_full}} (planos diferentes de 'Lite') são baseados no conceito de megabytes trocados no período de um mês. Esse
...
Para obter informações sobre o custo de cada megabyte de dados trocados, veja o serviço {{site.data.keyword.iot_short_notm}} no catálogo do {{site.data.keyword.Bluemix_notm}} para a região requerida.
...
Os itens a seguir são medidos e faturados de acordo com o uso: 
