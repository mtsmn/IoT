...
Get started with {{site.data.keyword.iot_full}} by using the {{site.data.keyword.iot_short_notm}} Starter GitHub project. By using the Starter, you can quickly simulate a device and generate data in the {{site.data.keyword.iot_short_notm}} dashboard.  
{:shortdesc}
...
Before you begin, you'll need an [IBM Cloud account](https://bluemix.net/registration) .

**Tip:** To make moving between tasks easier in the process that follows, make sure that you open the following pages in different tabs in your browser:  
- *{{site.data.keyword.Bluemix_notm}} dashboard*  
See the state of your deployment, read documentation, and launch the dashboards.
- *{{site.data.keyword.iot_short_notm}} dashboard*  

Define device types and register devices.

- *Node-RED*  
Configure and run the device simulator flow and work with other flows to process data from {{site.data.keyword.iot_short_notm}}.
...
1. Deploy the starter application.
 1. Click **Create Toolchain** to create a new Continuous Delivery Toolchain in Cloud.  
 **Tip:** If you'd rather deploy from the command line, you can [find the {{site.data.keyword.iot_short_notm}} starter](https://github.com/ibm-watson-iot/iot-platform-bluemix-starter) in the IBM Watson IoT organization in GitHub.
 2. When prompted, log in to IBM Cloud.
 3. If needed, select the Cloud Organization where you want to deploy the starter application.
 ...
 5. Click **Create**.  
**Tip:** Click the **Delivery Pipeline** tile to monitor the progress of your deployment.
 6. When the deployment is complete, click **View App** to open your new Node-RED application in a new tab.
2. Locate the starter application services.

 1. From the Cloud menu, select **Dashboard**.  
...
Complete the following steps to simulate a scenario that uses a thermostat to monitor the temperature, humidity, and location of a living room.

1.	Launch the {{site.data.keyword.iot_short_notm}} dashboard.
  1. In the Cloud dashboard, under *All services*, click the name of your {{site.data.keyword.iot_short_notm}} instance.
...
2. Create a device type.
  ...
  2.	On the Add Device page, click **Create device type**.
  3.	On the Create Device Type page, click **Create device type**.
  4. Enter a unique name, such as `Thermostat`, for your device type and click **Next**.
  5. Optional: Define a template and metadata or skip by clicking **Next** on each page.
  6.	Click **Create** to add the device type.  
On the Add Device page, the device type that you just created is displayed in the list of device types.
3.	Add a device that uses the newly created device type.
  1. Click **Next** to add a device that uses that device type.
  2. Enter a unique device ID, such as `LivingRoomThermo1`.
  3. Optional: Provide descriptive data on the Add Device page and enter device metadata or skip by clicking **Next** on each page.
...
4.	Make a note of the information that is displayed on the Your Device Credentials page.   
...
1. Launch the Node-RED flow editor.
  1. In the Cloud dashboard, under *All apps*, click the name of your toolchain.  
...
```
  ...
  8. On the {{site.data.keyword.iot_short_notm}} Device Information page, verify that you see data points from the device in the Sensor Information section.
...
1. Create a board
  ...
  **Tip:** If the {{site.data.keyword.iot_short_notm}} dashboard is not already open in another tab, return to your {{site.data.keyword.Bluemix_notm}} dashboard, click the name of your {{site.data.keyword.iot_short_notm}} instance, and then click **Launch Dashboard**.  
  2. Create a board to contain the cards for your simulated devices.
    ...
    2. Enter a name for the board, such as `Home Environment`, and click **Next**.
...
2. Create a card to display temperature
  ...
  2. Select your device from the devices list and then click **Next**.
  3. Click **Connect new data set**.
  4. On the Create Value Card page, select or enter the following values and click **Next**:
    - Event: update
    - Property: temp
    - Name: Temperature
    - Type: Float
    ...
    - Min: 0
    - Max: 50
  5. On the Card Preview page, select **L** for the line chart size, and click **Next**.
  6. On the Card Information page, change the name of the card to **Temperature** and click **Submit**.   
...
3. Create a card to display humidity
  ...
  2. Select your device from the list and then click **Next**.
  3. Click **Connect new data set**.
  4. On the Create Value Card page, select or enter the following values and click **Next**:
  Event: update
     - Property: humidity
     ...
     - Min: 10
     - Max: 95
  5. On the Card Preview page, select **M** for the gauge size and click **Next**.
  6. On the Card Information page, change the name of the card to **Humidity** and click **Submit**.   
...
<!-- 4. Create a card to display location.
  1. Click **Add New Card** and then select the **Value** card type, which is located in the Devics section.
  2. Select your device from the list and then click **Next**.
  3. Click **Connect new data set**.
  ...
  6. On the Create Value Card page, select or enter the following values and click **Next**:
     - Event: update
     ...
     - Min: -180
     - Max: 180
  7. On the Card Preview page, select **M** as the text size and click **Next**.
  8. On the Card Information page, change the name of the card to **Location** and click **Submit**.   
...
Node-RED continues to send data until you stop it. To stop the simulated data, perform the following steps:
    1.	In your Node-RED flow editor, double-click the gray **Send Data** node and set the Repeat value to **none**.
    2. Click **Done**.
